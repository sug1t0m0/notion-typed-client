...
describe('ClientGenerator - Pagination Methods', () => {
  ...
  describe('Code Generation', () => {
    ...
    it('should exclude start_cursor from args in new methods', () => {
      ...
      expect(code).toMatch(
        /queryDatabaseAll[\s\S]*?args\?: \{[\s\S]*?filter\?: GetFilterType<T>;[\s\S]*?sorts\?: Array<\{[\s\S]*?\}>;[\s\S]*?page_size\?: number;[\s\S]*?\}/
      );

      // queryDatabaseIterator should not accept start_cursor
      expect(code).toMatch(
        /queryDatabaseIterator[\s\S]*?args\?: \{[\s\S]*?filter\?: GetFilterType<T>;[\s\S]*?sorts\?: Array<\{[\s\S]*?\}>;[\s\S]*?page_size\?: number;[\s\S]*?\}/
      );
    });
